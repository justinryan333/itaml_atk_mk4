...
class ImageWithAttributes:
    def __init__(self, image, label):
        self.image = image
        self.label = label
        self.data = image  # or any other attributes you need to retain
        self.targets = label  # assuming label corresponds to targets for this example

    def __call__(self):
        return self.image, self.label  # Or whatever method you need to access the data
    ...
    def _setup_data(self, datasets, path, random_order=False, seed=1, increment=10, validation_split=0.):
        ...
        for dataset in datasets:
            ...
            # ===============================================================================================================================================================
                ...
                test_dataset = dataset.base_dataset(root=path, train=False, download=True, transform=trsf_test)
                poison_train_data = new_train_set
                train_dataset = [ (ImageWithAttributes(trsf_train(ToPILImage()(image)) if isinstance(image, torch.Tensor) else trsf_train(image),label)) for image, label in poison_train_data]
            else:
